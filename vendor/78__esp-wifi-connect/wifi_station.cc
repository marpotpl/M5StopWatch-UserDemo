...
void WifiStation::Stop() {
    ...
    // Now safe to stop scan, disconnect and stop WiFi
    // (no event callbacks will fire)
    esp_wifi_scan_stop();
    esp_wifi_disconnect();
    esp_wifi_stop();
    ...
    // Reset was_connected_ flag to prevent stale state
    // from affecting subsequent sessions
    ...
    // Set stopped event AFTER cleanup is complete
    // to unblock WaitForConnected
    xEventGroupSetBits(
        event_group_,
        WIFI_EVENT_STOPPED
    );
...
void WifiStation::Start() {
    // Note: esp_netif_init() and esp_wifi_init()
    // should be called once before calling this method.
    // WiFi driver is initialized by WifiManager::Initialize()
    // and kept alive.
    ...
    xEventGroupClearBits(
        event_group_,
        WIFI_EVENT_STOPPED | WIFI_EVENT_SCAN_DONE_BIT
    );
...
bool WifiStation::WaitForConnected(int timeout_ms) {
    ...
    auto bits = xEventGroupWaitBits(
        event_group_,
        WIFI_EVENT_CONNECTED | WIFI_EVENT_STOPPED,
        pdFALSE,
        pdFALSE,
        timeout_ms / portTICK_PERIOD_MS
    );

    // Return true only if connected
    // (not if stopped)
...
void WifiStation::HandleScanResult() {
    ...
    for (int i = 0; i < ap_num; i++) {
        ...
        if (it != ssid_list.end()) {
            ESP_LOGI(
                TAG,
                "Found AP: %s, BSSID: %02x:%02x:%02x:%02x:%02x:%02x, RSSI: %d, Channel: %d, Authmode: %d",
                (char *)ap_record.ssid,
                ap_record.bssid[0],
                ap_record.bssid[1],
                ap_record.bssid[2],
    ...
    if (connect_queue_.empty()) {
        ESP_LOGI(
            TAG,
            "No AP found, next scan in %d seconds",
            scan_current_interval_microseconds_
                / 1000
                / 1000
        );
...
void WifiStation::StartConnect() {
    auto ap_record =
        connect_queue_.front();

    connect_queue_.erase(
        connect_queue_.begin()
    );
...
void WifiStation::SetScanIntervalRange(
    int min_interval_seconds,
    int max_interval_seconds
) {
    scan_min_interval_microseconds_ =
        min_interval_seconds * 1000 * 1000;

    scan_max_interval_microseconds_ =
        max_interval_seconds * 1000 * 1000;

    scan_current_interval_microseconds_ =
        scan_min_interval_microseconds_;
}

void WifiStation::SetPowerSaveLevel(
    WifiPowerSaveLevel level
) {
    ...
    switch (level) {
        case WifiPowerSaveLevel::LOW_POWER:
            ...
            ESP_LOGI(
                TAG,
                "Setting WiFi power save level: LOW_POWER (MAX_MODEM)"
            );
        ...
        case WifiPowerSaveLevel::BALANCED:
            ...
            ESP_LOGI(
                TAG,
                "Setting WiFi power save level: BALANCED (MIN_MODEM)"
            );
        ...
        default:
            ...
            ESP_LOGI(
                TAG,
                "Setting WiFi power save level: PERFORMANCE (NONE)"
            );
...
void WifiStation::WifiEventHandler(
    void* arg,
    ...
    if (event_id == WIFI_EVENT_STA_START) {
        ...
        // WAŻNA ZMIANA:
        //
        // HandleScanResult() nie jest wykonywane bezpośrednio
        // w systemowym tasku sys_evt.
        //
        // Tworzymy własny task FreeRTOS z większym stosem.
        // ----------------------------------------------------
        ...
        if (result != pdPASS) {
            ESP_LOGE(
                TAG,
                "Failed to create wifi_scan_result task"
            );
        }
        ...
        // Notify disconnected callback only once
        // when transitioning from connected to disconnected
        bool was_connected =
            this_->was_connected_;
        ...
        ESP_LOGI(
            TAG,
            "No more AP to connect, next scan in %d seconds",
        ...
        // Nic tutaj nie robimy.
        // Adres IP obsługuje IpEventHandler().
    }
}

void WifiStation::IpEventHandler(
    void* arg,
    esp_event_base_t event_base,
    int32_t event_id,
    void* event_data
    ...
    // Reset scan interval to minimum
    // for fast reconnect if disconnected later
    this_->scan_current_interval_microseconds_ =
