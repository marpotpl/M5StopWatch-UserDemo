...
static int ws_read_header(esp_transport_handle_t t, char *buffer, int len, int timeout_ms)
{
    ...
    // HTTP Upgrade may have read the first WebSocket frame into ws->buffer.
    // In that case the socket itself need not be readable yet.
    if (ws->buffer_len == 0 && (poll_read = esp_transport_poll_read(ws->parent, timeout_ms)) <= 0) {
        return poll_read;
    }
